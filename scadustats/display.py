"""Rendering for the read-only `match` CLI commands (see cli.py).

Kept out of cli.py so the formatting is plain data-in/strings-out and can be tested
without a CliRunner: every function here returns lines (or one line) and prints nothing,
and cli.py's job is reduced to reading the JSON and echoing what comes back.
