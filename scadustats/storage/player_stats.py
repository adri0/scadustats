...
`<data_dir>/players` -- wholly regenerated by `player consolidate` on every run, and
gitignored along with the rest of `data_dir` since it's disposable output, not
hand-authored content.

This is the dynamic half of a player's profile split by issue #82: the hand-curated
identity half (id/twitch/avatar/bio) lives in storage.player_info instead, under a
directory meant to be committed to the repo -- this module carries no fields for it and
never touches it.
"""
...
def write_player_stats(players_dir: str | Path, stats: PlayerStats) -> Path:
    """Write one player's stats to `<players_dir>/<slug>.yaml` (see player_stats_path),
    creating players_dir if it doesn't exist yet. Unconditionally overwrites -- every
    field here is wholly regenerated by this run's consolidate_players output, with
    nothing hand-owned to protect (unlike storage.player_info.write_player_info).
...
def read_players_stats(players_dir: str | Path) -> dict[str, PlayerStats]:
    """Every player stats file already on disk under players_dir, keyed by slug --
    empty if the directory doesn't exist yet (a fresh checkout, or one `player
    consolidate` has never run against), the same "ships empty" tolerance
    ...
    if not directory.is_dir():
        return {}
    return {path.stem: read_player_stats(path) for path in sorted(directory.glob("*.yaml"))}
